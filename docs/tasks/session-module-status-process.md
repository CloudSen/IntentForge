...
## Acceptance Criteria
- [ ] 新增 `intentforge-session` 聚合模块，并完成 `intentforge-session-core` 与 `intentforge-session-local` 分层。
- [ ] `session-core` 提供清晰的会话管理领域模型、查询/写入契约、SPI Provider 接口及必要异常。
- [ ] `session-local` 提供内存版会话管理实现、classpath SPI 选择逻辑，以及覆盖正常、边界、非法输入、异常场景的测试。
- [ ] 根 POM、模块文档与架构文档完成更新，且 `make test` 通过。
- [ ] 任务文件持续记录 checkpoint，并与实际 git commit 保持一致。
